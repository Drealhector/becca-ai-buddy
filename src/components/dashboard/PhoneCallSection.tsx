          ...
          <div>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {incomingCalls.length === 0 ? (
                ...
                incomingCalls.map(renderCallCard)
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {outgoingCalls.length === 0 ? (
                ...
                outgoingCalls.map(renderCallCard)
              )}
            </div>
          </div>
        </div>
          ...
          <ScrollArea className="h-[400px] pr-4">
            {selectedCallTranscript ? (
              <div className="space-y-4">
                <div>
                  <p className="text-sm font-medium">Number: {selectedCallTranscript.number}</p>
                  <p className="text-sm text-muted-foreground">
                    {format(new Date(selectedCallTranscript.timestamp), "MMM dd, yyyy HH:mm")}
                  </p>
                  <p className="text-sm text-muted-foreground">
                ...
                <div className="bg-muted/50 p-4 rounded-lg">
                  <p className="text-sm whitespace-pre-wrap">
                    {selectedCallTranscript.topic || "No transcript available yet. Transcripts will appear here after calls are completed."}
                  </p>
                </div>
              </div>
            ) : (
